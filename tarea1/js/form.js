...
const update_region_select = () => {
    ...
    REGIONS.regiones.forEach((element, idx) => {
        region_select.innerHTML += `<option value="${idx}">${element.region}</option>`
    })        
...
const update_city_select = () => {
    ...
    const region_idx = region_select.value;
...
const validate_form = () => {
    ...
    const valid_name = form_name.value.length >= 3 && form_name.value.length < 80;
    const valid_email = form_email.value.match(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/) != null;
    const valid_phone = !form_phone.value || form_phone.value.replaceAll(' ','').match(/^(\+?56)?(\s?)(0?9)(\s?)[98765432]\d{7}$/) != null;

    const error_messages = [
        "Debes seleccionar un tipo de producto",
        "Debes seleccionar al menos un producto para el pedido",
        () => {
            if(file_input.files.length == 0){return "Debes seleccionar al menos un archivo"}
            else{return "Demasiados archivos seleccionados (máx. 3)"}
        },
        "Debes seleccionar una region",
        "Debes seleccionar una comuna",
        () => {
            if(form_name.value.length < 3){return "El nombre es demasiado corto (3 mín.)"}
    ...
    if(!error){
        ...
        div.innerHTML=`<div><p>¿Enviar ${value}?</p></div> <div><button type="button" class="button2" id="confirm">Confirmar</button> <button type="button" class="button2" id="deny">Volver al formulario</button></div>`;

        document.getElementById("confirm").addEventListener("click",() => {
            document.getElementById("form-container").innerHTML=`<h2>¡${value.charAt(0).toUpperCase() + value.slice(1, value.length)} añadido!</h2><h2><a style="text-decoration:underline" href="./index.html">¿Volver al Inicio?</h2>`;
